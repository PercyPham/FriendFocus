...
export const Header = () => {
  ...
  const MenuRow = ({
    ...
    >
      ...
      <div className='flex items-center gap-2'>
        {extra && (
          <span className='text-xs text-gray-400 font-normal leading-none'>
            {extra}
          </span>
        )}
  ...
  return (
    <div className='flex items-center justify-between px-6 py-4 bg-white dark:bg-slate-900 border-b border-gray-100 dark:border-slate-800 sticky top-0 z-20 transition-colors'>
      <div className='flex items-center gap-2'>
          ...
          <div className='absolute right-0 top-full mt-2 w-64 bg-white dark:bg-slate-800 rounded-xl shadow-2xl border border-gray-100 dark:border-slate-700 animate-in fade-in slide-in-from-top-2 z-30 p-1.5'>
            <>
              ...
              <MenuRow
                ...
                label='Send Feedback'
                isLink
                href={LINKS.SEND_FEEDBACK}
              />
              <MenuRow
                icon={Bug}
                label='Report a Bug'
                isLink
                href={LINKS.REPORT_A_BUG}
              />
              <div className='h-px bg-gray-100 dark:bg-slate-700 my-1' />
